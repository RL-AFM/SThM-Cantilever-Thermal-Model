...
def constructMetalDicts(padded_dict):
    ...
    for key,value in padded_dict.iteritems():
        if key=='Pd':
            ...
            Pd['width']=transform_heater(value/2.)    #correct width in direction of current flow for 45 degree angle
            ...
            Pd['elec_resistance']=[rhoOverA[count]*dxprime if (dx*count)<=139e-6 else rhoOverA[count]*tipTilt(dxprime) for count in range(0,len(Pd['width']))]
            ...
            Pd['power']=(2e-3**2)*Pd['elec_resistance']*(Pd['width']/Pd['fixedwidth'])*2.
            ...
            numerator = np.asarray([Au['resistivity']*dx if dx*count<=139e-6 else Au['resistivity']*tipTilt(dx) for count in range(0,len(Au['width']))])
            ...
            Au['power']=2e-3**2*Au['elec_resistance']
...
sum_power=constructMetalDicts(padded)
